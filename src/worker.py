...
async def make_payment_request(payment_payload, processors, max_attempts=3) -> str:
    urls = {
...
async def payment_worker():
    while True:
        ...
        if payment_request is None:
            break
        await process_payment(payment_request, send_retry_queue=True)
        globals.payment_queue.task_done()


async def retry_payment_worker():
    while True:
        payment_request = await globals.retry_payment_queue.get()
        if payment_request is None:
            break
        await process_payment(payment_request, send_retry_queue=False)
        globals.retry_payment_queue.task_done()


async def process_payment(payment_request, send_retry_queue=True):
    requested_at = datetime.now(timezone.utc)
    payment_request["requestedAt"] = requested_at.isoformat()
    try:
        processors = await set_best_payment_processor()
        payment_processor = await make_payment_request(payment_request, processors=processors)
        payment = Payment(
        ...
        if send_retry_queue:
            globals.logger.warning("Payment request failed. Sending to retry queue.")
            await globals.retry_payment_queue.put(payment_request)
